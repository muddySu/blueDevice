...
@synthesize managerbluetooth;          //主设备
@synthesize activePeripheral;      //从设备cbperipheral类对象

@synthesize timer;

/*-------------单例设计模式---------------*/
static SerialGATT *sencor = nil;

+(SerialGATT *)sharedInstanceMethod
{
    @synchronized(self){
        if ( sencor == nil) {
            sencor = [[SerialGATT alloc] init];
        }
    }
    return sencor;
}
...
+(id)allocWithZone:(NSZone *)zone
{
    @synchronized(self)
    {
        if(sencor == nil)
        {
            sencor  = [super allocWithZone:zone];
            return sencor;
        }
    }
    return nil;
...
//初始化  CBCentralManager 设置其代理给SerialGATT
-(void) setup
{
    managerbluetooth = [[CBCentralManager alloc] initWithDelegate:self queue:nil];
}
...
{
    if ([managerbluetooth state] != CBCentralManagerStatePoweredOn) {
    ...
    [managerbluetooth scanForPeripheralsWithServices:nil options:0];
    //所有发现的从设备被返回 
    return 0;
}
...
-(void) connect:(CBPeripheral *)peripheral
{
    if (![peripheral isConnected]) {
        [managerbluetooth connectPeripheral:peripheral options:nil];
        NSLog(@"连接中");
    }
}
...
{
    [managerbluetooth cancelPeripheralConnection:peripheral];
}

#pragma mark - basic operations for SerialGATT service
-(void) write:(CBPeripheral *)peripheral data:(NSData *)data
{
    [self writeValue:fileService characteristicUUID:fileRead p:peripheral data:data];
...
-(void) notify: (CBPeripheral *)peripheral on:(BOOL)on
{
    ...
    //[self notification:fileService characteristicUUID:fileRead p:peripheral on:YES];
...
//                                                 target:self
//                                               selector:@selector(readRSSIHere)
//                                               userInfo:nil
//                                                repeats:1.0];
...
-(void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error
{
   // activePeripheral = nil;
    ...
    [[[UIAlertView alloc] initWithTitle:@"welcome" message:@"连接断开，请重新尝试连接" delegate:self cancelButtonTitle:@"done" otherButtonTitles: nil] show];
...
-(void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    //[delegate serialGATTCharValueUpdated:@"2AF1" value:characteristic.value];
    
    ViewController *mycontrol = [ViewController sharedInstanceMethod];
    [mycontrol getCharValueUpdated:@"FFF2" value:characteristic.value];
